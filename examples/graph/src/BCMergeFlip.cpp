...
BCMergeFlip<K,V>::BCMergeFlip(std::string dir, std::string old_name, int toflip_fd) {
...
BCMergeFlip<K,V>::Next() {
	...
	if ( mp_old_reader != NULL ) {
		if ( std::get<2>(cur_buffered_val) ) {
			...
			if (std::get<0>(ret) == std::get<0>(cur_buffered_val)) {
			...
			} else if ( std::get<0>(ret) > std::get<1>(cur_buffered_val) ) {
				std::tuple<V,K, bool> rold = cur_buffered_val;
				cur_buffered_val = mp_old_reader->Next();
