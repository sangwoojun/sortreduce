...
int main(int argc, char** argv) {
	...
	if ( argc < 4 ) {
		fprintf(stderr, "usage: %s directory iter_count element_count\n", argv[0] );
		exit(1);
	}
	...
	printf( "Iter count: %d Element count: %lu\n", iter_count, element_count );



	for ( int iter = 0; iter < iter_count; iter++ ) {
		SortReduceTypes::Config<uint64_t,uint32_t>* conf = new SortReduceTypes::Config<uint64_t,uint32_t>(tmp_dir, "output.dat", 8);
		//SortReduceTypes::Config<uint64_t,uint32_t>* conf = new SortReduceTypes::Config<uint64_t,uint32_t>(tmp_dir, "", 8);
